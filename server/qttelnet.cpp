...
void QtTelnetPrivate::sendWindowSize()
{
    ...
    short h = (short)(windowSize.height());
    short w = (short)(windowSize.width());
...
void QtTelnet::connectToHost(const QString &host, quint16 port)
{
    ...
    if(!d->socket->waitForConnected(1000))
    {
        qDebug() << host << "can't be connected";
